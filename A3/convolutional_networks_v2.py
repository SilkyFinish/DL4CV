...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        self.params[f'W{self.num_layers}'] = torch.normal(0, weight_scale, size=(num_filters[-1] * H * W, num_classes), dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            # Check if the layer should include batch normalization
            if self.batchnorm:
                out, caches[f'Conv_BN_ReLU_Pool{i}'] = Conv_BatchNorm_ReLU_Pool.forward(
                    out, 
                    self.params[f'W{i}'], 
                    self.params[f'b{i}'], 
                    ...
                    conv_param, 
                    self.params[f'bn_param{i}'], 
                    pool_param
                )
            else:
                out, caches[f'Conv_ReLU_Pool{i}'] = Conv_ReLU_Pool.forward(
                    out, 
                    self.params[f'W{i}'], 
                    self.params[f'b{i}'], 
                    conv_param, 
        ...
        for i in list(range(1, self.num_layers))[::-1]:
            if self.batchnorm:
                dx, dw, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dx, caches[f'Conv_BN_ReLU_Pool{i}'])
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dbeta = dout.sum(dim=0)  # dbeta is the gradient of the loss w.r.t beta
        dgamma = torch.sum(dout * x_normalized, dim=0)  # dgamma is the gradient of the loss w.r.t gamma

        # Step 2: Calculate the gradient with respect to the normalized input
        dx_normalized = dout * gamma  # Gradient of the loss w.r.t x_normalized

        # Step 3: Backprop through normalization
        # Using chain rule to calculate the gradients with respect to x
        dvar = torch.sum(dx_normalized * (x - sample_mean) * -0.5 * (sample_var + eps)**(-3/2), dim=0)
        dmean = torch.sum(dx_normalized * -1 / torch.sqrt(sample_var + eps), dim=0) + dvar * torch.mean(-2 * (x - sample_mean), dim=0)
    ...
    def backward_alt(dout, cache):
        ...
        # Step 1: Calculate dbeta and dgamma
        dbeta = dout.sum(dim=0)  # dbeta: gradient of loss w.r.t. beta
        dgamma = torch.sum(dout * x_normalized, dim=0)  # dgamma: gradient of loss w.r.t. gamma

        # Step 2: Calculate the gradient w.r.t. the normalized input
        dx_normalized = dout * gamma  # Gradient of loss w.r.t. x_normalized

        # Step 3: Backprop through normalization
        dvar = torch.sum(dx_normalized * (x - sample_mean) * -0.5 * (sample_var + eps)**(-3/2), dim=0)
        dmean = torch.sum(dx_normalized * -1 / torch.sqrt(sample_var + eps), dim=0) + dvar * torch.mean(-2 * (x - sample_mean), dim=0)

        # Step 4: Calculate dx (gradient w.r.t. input x)
        dx = (dx_normalized / torch.sqrt(sample_var + eps)) + (dvar * 2 * (x - sample_mean) / N) + (dmean / N)
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Step 1: Reshape input to (N * H * W, C)
        x_reshaped = x.transpose(0, 1).reshape(C, -1).T  # Now it's of shape (N*H*W, C)

        # Step 2: Apply the batch normalization on the reshaped input
        out_reshaped, cache = BatchNorm.forward(x_reshaped, gamma, beta, bn_param)

        # Step 3: Reshape the output back to (N, C, H, W)
        out = out_reshaped.T.reshape(C, N, H, W).transpose(0, 1)
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        # Step 1: Reshape dout to (N * H * W, C)
        dout_reshaped = dout.transpose(0, 1).reshape(C, -1).T  # Now it's of shape (N*H*W, C)

        # Step 2: Call the vanilla backward pass
        dx_reshaped, dgamma, dbeta = BatchNorm.backward(dout_reshaped, cache)

        # Step 3: Reshape dx back to (N, C, H, W)
        dx = dx_reshaped.T.reshape(C, N, H, W).transpose(0, 1)
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
