...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################
        # Replace "pass" statement with your code
        # Retrieve the input 'x' from cache
        x = cache
        
        # Initialize dx with the same shape as x
        dx = dout.clone()  # Create a copy of dout to avoid modifying it in-place
        
        # Apply the ReLU backward logic: zero-out dx where x <= 0
        dx[x <= 0] = 0
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        out_l1,cache1=Linear.forward(X,self.params['W1'],self.params['b1'])
        out_l2,cache2=ReLU.forward(out_l1)
        scores,cache3=Linear.forward(out_l2,self.params['W2'],self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dx,dw2,db2=Linear.backward(dx,cache3)
        dx = ReLU.backward(dx, cache2)
        dx,dw1,db1=Linear.backward(dx,cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W1'] = torch.normal(0, weight_scale, size=(input_dim,hidden_dims[0]), dtype=dtype, device=device)
        self.params['b1'] = torch.zeros(hidden_dims[0], dtype=dtype, device=device)
        for i in range(1,hidden_nums):
            self.params[f'W{i+1}'] = torch.normal(0, weight_scale, size=(hidden_dims[i-1],hidden_dims[i]), dtype=dtype, device=device)
            self.params[f'b{i+1}'] = torch.zeros(hidden_dims[i], dtype=dtype, device=device)
        self.params[f'W{hidden_nums+1}'] = torch.normal(0, weight_scale, size=(hidden_dims[hidden_nums-1],num_classes), dtype=dtype, device=device)
        self.params[f'b{hidden_nums+1}'] = torch.zeros(num_classes, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        dropout_caches = {}  # 存储dropout层的cache
        out = X.reshape(X.shape[0], -1)  # 将输入数据展平成二维矩阵 (N, D)

        # 前L-1层的前向传播
        for i in range(1, self.num_layers):
            # 线性层
            out, caches[f'linear{i}'] = Linear.forward(out, self.params[f'W{i}'], self.params[f'b{i}'])
            
            # ReLU激活
            out, caches[f'relu{i}'] = ReLU.forward(out)
            
            # 如果使用了dropout，在relu之后进行dropout
            if self.use_dropout:
                out, dropout_caches[i] = torch.nn.functional.dropout(out, p=self.dropout_param['p'], training=mode == 'train')

        # 最后一层（L层，不带ReLU）
        scores, caches[f'linear{self.num_layers}'] = Linear.forward(out, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in list(range(2,self.num_layers+1))[::-1]:
            dx,dw,db=Linear.backward(dx,caches[f'linear{i}'])
            dx = ReLU.backward(dx, caches[f'relu{i-1}'])
            grads[f'W{i}'] = dw + 2*self.reg * self.params[f'W{i}']  # Add L2 regularization
            grads[f'b{i}'] = db
        dx,dw,db=Linear.backward(dx,caches['linear1'])
        grads['W1'] = dw + 2*self.reg * self.params['W1']  # Add L2 regularization
        grads['b1'] = db
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(
        model, 
        data=data_dict,  # 传入训练和验证数据集
        optim_config={
            'learning_rate': 1e-3,  # 设置学习率
        },
        lr_decay=0.95,  # 学习率衰减
        num_epochs=10,  # 设置训练的 epoch 数
        batch_size=100,  # 每个 minibatch 的大小
        print_every=100,  # 每100次打印损失
        device=device  # 运行设备 'cpu' 或 'cuda'
    )
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
...
def sgd_momentum(w, dw, config=None):
    ...
    ##################################################################
    # Replace "pass" statement with your code
    # Ensure w and dw are valid tensors
    assert isinstance(w, torch.Tensor), "w should be a torch.Tensor"
    assert isinstance(dw, torch.Tensor), "dw should be a torch.Tensor"
    assert w.shape == dw.shape, "Shape of w and dw should match"
...
def rmsprop(w, dw, config=None):
    ...
    cache.mul_(config['decay_rate']).add_((1 - config['decay_rate']) * dw ** 2)

    # Compute the next weight
    next_w = w - (config['learning_rate'] / (torch.sqrt(cache) + config['epsilon'])) * dw

    # Store the updated cache back in config
    config['cache'] = cache
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    t=config['t']
    t+=1
    m=beta1*m+(1-beta1)*dw #similar to velocity
