...
def svm_loss_vectorized(
    ...
    margins = torch.clamp(margins, min=0)  # shape: (N, C)
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-2,2e-1, 1e-4]
    regularization_strengths = [1e0, 1e1,1e2,2e1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    cls.K= 0.000001 * torch.randn(X_train.shape[1], num_classes, device=X_train.device, dtype=X_train.dtype)
    loss_history = cls.train(X_train, y_train, learning_rate=lr, reg=reg, num_iters=num_iters)
    train_predictions = cls.predict(X_train)
    val_predictions = cls.predict(X_val)
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        loss+=-torch.log(torch.exp(scores[y[i]]/denom))
    loss /= num_train
    loss += reg * torch.sum(W * W)
    for j in range(num_classes):
            softmax_output = torch.exp(scores[j]) / denom
            if j == y[i]:
                dW[:, j] += (softmax_output - 1) * X[i]  # Correct class gradient
            else:
                dW[:, j] += softmax_output * X[i]  # Other classes gradient
    dW/=num_train
    dW+=2*reg*W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_loss_vectorized(
    ...
    denom=torch.sum(exp_scores,dim=1)
    probs = exp_scores / denom
    correct_probs=probs[torch.arange(num_train),y]
    softmax=-torch.log(correct_probs)
    loss=softmax.sum()/num_train+reg * torch.sum(W * W)
    # 计算概率偏导数
    dscores = probs.clone()
    dscores[torch.arange(num_train), y] -= 1  # 对正确类别，减去 1
    dscores /= num_train  # 均值化

    # 计算 dW: X.T * dscores
    dW = torch.mm(X.t(), dscores)
