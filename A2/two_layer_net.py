...
def nn_forward_backward(
    ...
    loss=softmax.sum()/N+reg * (torch.sum(W1 * W1)+torch.sum(W2 * W2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    # 计算概率偏导数
    dscores = probs.clone()
    dscores[torch.arange(N), y] -= 1  # 对正确类别，减去 1
    dscores /= N  # 均值化
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-2,1e-1,1e-4,1e-5]
    hidden_sizes = [8, 16,32,64,128]
    regularization_strengths = [1e0, 1e1,1e2,1e3,1e4]
    learning_rate_decays = [1.0, 0.95,0.9,0.85,0.8]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    # Get hyperparameter sets from the provided function
    learning_rates, hidden_sizes, regularization_strengths, learning_rate_decays = get_param_set_fn()

    # Nested loops to iterate over each hyperparameter combination
    for lr in learning_rates:
        for hidden_size in hidden_sizes:
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    # Initialize a new TwoLayerNet model with the current set of hyperparameters
                    net = TwoLayerNet(input_size=X_train.shape[1], hidden_size=hidden_size, output_size=10)
                    ...
                    # Get validation accuracy for the trained network
