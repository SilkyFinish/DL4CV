...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    m = nn.Tanh()
    next_h=m(torch.mm(x,Wx)+torch.mm(prev_h,Wh)+b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    # Backward through the tanh activation function
    da = dnext_h * (1 - next_h ** 2)  # Shape (N, H)

    dx = torch.matmul(da, Wx.T)  # Gradient with respect to input x, shape (N, D)
    dprev_h = torch.matmul(da, Wh.T)  # Gradient with respect to previous hidden state h, shape (N, H)
    dWx = torch.matmul(x.T, da)  # Gradient with respect to Wx, shape (D, H)
    dWh = torch.matmul(prev_h.T, da)  # Gradient with respect to Wh, shape (H, H)
    db = torch.sum(da, dim=0)  # Gradient with respect to bias b, shape (H,)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(0,T):
      xi=x[:,t,:].reshape(N,-1)
      next_h,cache_t=rnn_step_forward(xi,next_h, Wx, Wh, b)
      h[:,t,:]=next_h
      cache[t] = cache_t
    ##########################################################################
    #                             END OF YOUR CODE                           #
...
def rnn_backward(dh, cache):
    ...
    # Replace "pass" statement with your code
    N, T, H = dh.shape
    D = cache[0][0].shape[-1]  # Infer D from the first element in the cache
    ...
    db = torch.zeros(H)
    dh0 = torch.zeros((N, H))
    dprev_h = torch.zeros((N, H))  # Gradient for the hidden state from the next timestep

    # Iterate backwards through the sequence
    for t in reversed(range(T)):
        # Combine upstream gradient at time t with gradient from next timestep
        dcurrent_h = dh[:, t, :] + dprev_h
        dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dcurrent_h, cache[t])
    ...
    # dh0 is the gradient with respect to the initial hidden state
    dh0 = dprev_h
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = torch.nn.functional.cross_entropy(x.reshape(-1, x.size(-1)), y.reshape(-1), ignore_index=ignore_index, reduction='sum') / x.size(0)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        ######################################################################
        # Replace "pass" statement with your code
        self.feature_extract = ImageEncoder(pretrained=image_encoder_pretrained, verbose=False)
        input_dim = self.feature_extract.out_channels
        ...
        # Output projection layer to predict vocabulary probabilities
    ...
    def forward(self, images, captions):
        ...
        # Replace "pass" statement with your code
        # Replace "pass" statement with your code
        image_feature=self.feature_extract(images)
        image_feature = self.adaptive_pool(image_feature).view(images.size(0), -1)  # Shape (B, out_channels)
        # Step 2: Extract features from images and project to initial RNN hidden state
        # Assuming self.feature_projection is a linear layer that projects image features to RNN hidden size
        h0 = self.feature_projection(image_feature)  # Shape (N, H) for RNN/LSTM or (N, H, 4, 4) for Attention LSTM

        # Step 3: Embed the input captions
        embedded_captions = self.word_embedding(captions_in)  # Shape (N, T, W)

        # Step 4: Forward pass through RNN/LSTM
        if self.cell_type == "rnn":
            hiddens = self.rnn(embedded_captions, h0)  # Shape (N, T, H)

        # Step 5: Compute scores over vocabulary using the hidden states
        scores = self.output_projection(hiddens)  # Shape (N, T, V)

        # Step 6: Compute loss with temporal softmax, ignoring <NULL> tokens
    ...
    def sample(self, images, max_length=15):
        ...
        # Step 2: Extract features from images and project to initial RNN hidden state
        # Assuming self.feature_projection is a linear layer that projects image features to RNN hidden size
        next_h = self.feature_projection(image_feature)  # Shape (N, H) for RNN/LSTM or (N, H, 4, 4) for Attention LSTM

        # Start the sequence with the <START> token
        current_word = torch.full((N,), self._start, dtype=torch.long, device=images.device)

        for t in range(max_length):
            # Step 2: Embed the current word
            embedded_caption = self.word_embedding(current_word)  # Shape (N, wordvec_dim)

            # Step 3: RNN or LSTM step
            if self.cell_type == "rnn":
                next_h = self.rnn.step_forward(embedded_caption, next_h)

            # Step 4: Compute scores over the vocabulary
            scores = self.output_projection(next_h)  # Shape (N, vocab_size)

            # Step 5: Select the word with the highest score
            current_word = scores.argmax(dim=1)  # Shape (N,)

            # Save the selected word in the captions array
            captions[:, t] = current_word

            # Optional: Break if all sequences have generated <END> token
            # if (current_word == self._end).all():
            #     break
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(0,T):
          xi=x[:,t,:].reshape(N,-1)
          next_h,next_c=self.step_forward(xi,next_h,next_c)
          hn[:,t,:]=next_h
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # Calculate the dot product between prev_h and A
    # We need to add an extra dimension to prev_h to match the dimensions for broadcasting
    prev_h_expanded = prev_h.unsqueeze(-1).unsqueeze(-1)  # Shape: (N, H, 1, 1)
    dot_product = torch.matmul(A, prev_h_expanded)  # Shape: (N, H, 4, 4) -> (N, 1, 4, 4)
    
    # Scale the dot product
    scale = torch.sqrt(torch.tensor(H, dtype=dot_product.dtype, device=dot_product.device))
    scaled_dot_product = dot_product / scale
    
    # Apply softmax to get attention weights
    attn_weights = F.softmax(scaled_dot_product, dim=1)  # Softmax over H dimension
    
    # Multiply attention weights with A to get attention embedding output
    attn = torch.matmul(attn_weights, A)  # Shape: (N, 1, 4, 4) * (N, H, 4, 4) -> (N, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
