...
def get_subsequent_mask(seq):
    ...
    subsequent_mask = torch.triu(torch.ones((K, K), dtype=torch.bool), diagonal=1)
    # Expand this mask for each sequence in the batch
    mask = subsequent_mask.unsqueeze(0).expand(N, -1, -1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
