...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            x_i=x_train_flatten[i]
            x_j=x_test_flatten[j]
            dist=((x_i-x_j)**2).sum()
            dists[i,j]=dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        x_i=x_train_flatten[i]
        dist=((x_i-x_test_flatten)**2).sum(dim=1)
        dists[i]=dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten=x_test.view(num_test,-1)
    # 计算 x_train 的平方和
    x_train_square = (x_train_flatten ** 2).sum(dim=1, keepdim=True)

    # 计算 x_test 的平方和
    x_test_square = (x_test_flatten** 2).sum(dim=1, keepdim=True).t()

    # 计算 x_train 和 x_test 的点积
    dot_product = x_train_flatten @ x_test_flatten.t()

    # 使用广播计算欧几里得距离的平方
    dists = x_train_square + x_test_square - 2 * dot_product
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      ...
      Label=[]
      for index in idx:
        Label.append(y_train[index].item())
      dict={}
      for label in Label:
        if label in dict:
          dict[label]+=1
      ...
      for key in dict.keys():
        if dict[key]>max_value or (dict[key]==max_value and key<max_key):
          max_key=key
          max_value=dict[key]
      y_pred[i]=max_key
    ##########################################################################
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    num_train=x_train.shape[0]
    piece=num_train//num_folds
    for tensor in torch.chunk(x_train,num_folds,dim=0):
        x_train_folds.append(tensor)
    for tensor in torch.chunk(y_train,num_folds):
        y_train_folds.append(tensor)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train_data=torch.cat(x_train_folds[:i]+x_train_folds[i+1:],dim=0)
            x_test_data=x_train_folds[i]
            y_train_data=torch.cat(y_train_folds[:i]+y_train_folds[i+1:],dim=0)
            y_test_data=y_train_folds[i]
            classifier = KnnClassifier(x_train_data, y_train_data)
            fold_list.append(classifier.check_accuracy(x_test_data, y_test_data, k))
        k_to_accuracies[k]=fold_list
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    Label=list(k_to_accuracies.keys())
    max_key=Label[0]
    max_value=sum(k_to_accuracies[max_key])
    for key in Label:
        value=sum(k_to_accuracies[key])
